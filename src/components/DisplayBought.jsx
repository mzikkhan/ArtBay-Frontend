...
const DisplayBought = ({ title, isLoading, campaigns }) => {
  const navigate = useNavigate();
  const [searchTerm, setSearchTerm] = useState('');

  const handleNavigate = (campaign) => {
    navigate(`/bought-details/${campaign.title}`, { state: campaign })
  }

// Filter campaigns based on search term
const filteredCampaigns = campaigns.filter((campaign) =>
  campaign.seller.toLowerCase().includes(searchTerm.toLowerCase()) ||
  campaign.buyer.toLowerCase().includes(searchTerm.toLowerCase())
);
